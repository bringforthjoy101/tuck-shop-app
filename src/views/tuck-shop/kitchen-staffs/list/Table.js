...
const UsersList = () => {
  ...
  const filteredData = store.allData.filter( 
    student => (student.otherName?.toLowerCase() || student.firstName?.toLowerCase() || student?.lastName?.toString())
  )
  ...
  return (
    <Fragment>
      ...
      >
        {
          userData?.role === 'manager' || userData?.role === 'bursary' ? <Button.Ripple color='primary' onClick={toggleSidebar}> Add New Kitchen Staff </Button.Ripple> : ''
        }
      </Col>
      </Row>
        <DataTable
