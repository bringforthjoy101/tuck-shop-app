...
// ** Store & Actions
import { getUserAllUtilitiesTransactions, getFilteredStudentOrders } from '../../store/action'
		...
		<div className="invoice-list-table-header w-100 py-2">
			<Row>
				<Col lg="6" sm="12" className="d-flex align-items-center px-0 px-lg-1 mb-2">
					...
					<h3 className="d-none d-lg-block">{studentDetails.firstName}'s Orders</h3>
				</Col>
				<Col
					lg="3"
					sm="6"
...
const TransactionList = () => {
	...
	useEffect(() => {
		...
		dispatch(
			getFilteredStudentOrders(store.studentDetails.orders, {
				page: currentPage,
				perPage: rowsPerPage,
				q: searchTerm,
			})
	...
	const filteredData = store?.studentDetails.orders?.filter((item) => item?.orderNumber?.toLowerCase() || item?.status?.toLowerCase())

	const CustomPagination = () => {
		const count = Math.ceil(store?.studentDetails.orders.length / rowsPerPage)
	...
	function convertArrayOfObjectsToCSV(array) {
		...
		const lineDelimiter = '\n'
		const keys = Object.keys(store.studentDetails.orders[0])
	...
	const downloadPDF = () => {
		...
		doc.save(`${store.studentDetails.firstName} ${store.studentDetails.lastName}_orders.pdf`)
	}
	...
	const dataToRender = () => {
		...
		if (store.studentDetails.orders.length > 0) {
			return store.studentDetails.orders
		} else if (store.studentDetails.orders.length === 0 && isFiltered) {
			return []
		} else {
			return store.studentDetails.orders.slice(0, rowsPerPage)
		}
	}
					...
					<DataTable
						...
						subHeaderComponent={
							<CustomHeader
								...
								storeData={store.studentDetails.orders}
