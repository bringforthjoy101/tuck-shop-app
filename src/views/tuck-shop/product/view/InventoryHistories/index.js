...
const InventoryHistoryList = ({inventoryHistories, product}) => {
	...
	useEffect(() => {
		...
		dispatch(
			getFilteredinventoryHistories(inventoryHistories, {
				page: currentPage,
				perPage: rowsPerPage,
				q: searchTerm,
			})
	...
	const filteredData = inventoryHistories?.filter((item) => item?.department?.toLowerCase() || item?.type?.toLowerCase())
	...
	function convertArrayOfObjectsToCSV(array) {
		...
		const lineDelimiter = '\n'
		const keys = Object.keys(inventoryHistories[0])
	...
	const dataToRender = () => {
		...
		} else if (inventoryHistories?.length === 0 && isFiltered) {
			return []
		} else {
			return inventoryHistories?.slice(0, rowsPerPage)
		}
	}
					...
					<DataTable
						...
						subHeaderComponent={
							<CustomHeader
								...
								storeData={inventoryHistories}
								downloadPDF={downloadPDF}
								searchTerm={searchTerm}
								InventoryDetails={product}
							/>
						}
					/>
				</div>
