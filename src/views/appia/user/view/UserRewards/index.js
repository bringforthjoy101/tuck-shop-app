...
// ** Store & Actions
import { getUserRewards, getFilteredUserReward } from '../../store/action'
...
const TransactionList = () => {
  ...
  useEffect(() => {
    dispatch(getUserRewards(store.userDetails.user_details.user_id))
    dispatch(
      getFilteredUserReward(store.selectedUserRewardTransactions, {
        page: currentPage,
        perPage: rowsPerPage,
        q: searchTerm
      })
  ...
  const filteredData = store?.selectedUserRewardTransactions?.filter(
  ...
  function convertArrayOfObjectsToCSV(array) {
    ...
    const lineDelimiter = '\n'
    const keys = Object.keys(store.selectedUserRewardTransactions[0])
  ...
  const downloadPDF = () => {
    ...
    store.selectedUserRewardTransactions.map(arr => {
      doc.autoTable({
        styles: { halign: 'left' },
        columnStyles: {
          0: {cellWidth: 40},
  ...
  const dataToRender = () => {
    ...
    if (store.selectedUserRewardTransaction.length > 0) {
      return store.selectedUserRewardTransaction
    } else if (store.selectedUserRewardTransaction.length === 0 && isFiltered) {
      return []
    } else {
      return store.selectedUserRewardTransactions.slice(0, rowsPerPage)
    }
  }
          ...
          <DataTable
            ...
            subHeaderComponent={
              <CustomHeader
                ...
                storeData={store.selectedUserRewardTransactions}
