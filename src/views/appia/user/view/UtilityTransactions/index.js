...
// ** Store & Actions
import { getUserAllUtilitiesTransactions, getFilteredUserUtilityTransactions } from '../../store/action'
...
const TransactionList = () => {
  ...
  useEffect(() => {
    ...
    dispatch(
      getFilteredUserUtilityTransactions(store.selectedUserUtilitiesTransactions, {
        page: currentPage,
        perPage: rowsPerPage,
        q: searchTerm
      })
  ...
  const filteredData = store?.selectedUserUtilitiesTransactions?.filter(
  ...
  function convertArrayOfObjectsToCSV(array) {
    ...
    const lineDelimiter = '\n'
    const keys = Object.keys(store.selectedUserUtilitiesTransactions[0])
  ...
  const downloadPDF = () => {
    ...
    doc.autoTable({
        ...
        head: [['Id', 'Type', 'Amount', 'Description', 'Balance', 'Date']]
    })
  ...
  const dataToRender = () => {
    ...
    if (store.selectedUserUtilityTransactions.length > 0) {
      return store.selectedUserUtilityTransactions
    } else if (store.selectedUserUtilityTransactions.length === 0 && isFiltered) {
      return []
    } else {
      return store.selectedUserUtilitiesTransactions.slice(0, rowsPerPage)
    }
  }
