...
// ** Store & Actions
// import { getUserAllBankTransactions, getFilteredUserBankTransactions } from '../../store/action'
...
const TransactionList = () => {
  ...
  useEffect(() => {
    // dispatch(getUserAllBankTransactions(store.userDetails.user_details.user_id))
    // dispatch(
  ...
  function convertArrayOfObjectsToCSV(array) {
    ...
    const lineDelimiter = '\n'
    const keys = Object.keys(store.selectedUserBanksTransactions[0])
  ...
  const downloadPDF = () => {
    ...
    store.selectedUserBanksTransactions.map(arr => {
      doc.autoTable({
        styles: { halign: 'left' },
        columnStyles: {
          0: {cellWidth: 40},
  ...
  const dataToRender = () => {
    ...
    if (store.selectedUserBankTransactions.length > 0) {
      ...
      return store.selectedUserBanksTransactions.slice(0, rowsPerPage)
    }
  }
